...
from paraKang.common_func import (fmt_traceback, get_output_file_name,
...
class ParaKangTask(Task):
	...
	def __call__(self, *args, **kwargs):
		...
		self.results_dir = ctx.get('results_dir', PARAKANG_RESULTS)
		...
		if PARAKANG_CACHE_ENABLED:
			...
			if result and result != b'null':
				...
				if PARAKANG_RECORD_ENABLED and self.track:
		...
		# MED-01 fix: Use self.result (task execution result), not result (cache lookup variable)
		if PARAKANG_CACHE_ENABLED and self.status == SUCCESS_TASK and self.result:
	...
	def notify(self, name=None, severity=None, fields={}, add_meta_info=True):
		...
		from paraKang.tasks import send_task_notif
