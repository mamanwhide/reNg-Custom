...
from paraKang.settings import LOGGING
...
def bulk_import_targets(
	...
	""" 
		Used to import targets in paraKang
	...
	for target in targets:
		...
		if is_domain:
			...
			logger.warning(f'{name} is not supported by paraKang')
			continue
	...
	# MED-09 fix: Move organization association OUTSIDE the loop
	# Previously this ran inside the for-target loop, causing O(n²)
	# re-additions and variable shadowing
	if organization_name and all_targets:
		...
		# Associate all targets with the organization (once, not per iteration)
...
def store_domain(domain_name, project, description, h1_team_handle):
	"""
		This function is used to store domain in paraKang
	"""
