...
from paraKang.common_serializers import *
...
from paraKang.utilities import is_valid_url
...
def get_http_urls(
	...
	if ignore_files: # ignore all files
		extensions_path = f'{PARAKANG_HOME}/fixtures/extensions.txt'
...
def get_cms_details(url):
	...
	Returns:
		dict: Response.
	"""
	# HIGH-01 fix: Initialize response dict to avoid NameError
	response = {'status': False}

	# CRT-07 fix: Use subprocess.run() with list args instead of os.system()
	# Validate URL before passing to command
	...
	if not parsed_check.scheme or not parsed_check.hostname:
		...
		return response
	cms_detector_command = [
		'python3', '/usr/src/github/CMSeeK/cmseek.py',
		'--random-agent', '--batch', '--follow-redirect',
		'-u', url
	]
	try:
		subprocess.run(
		...
		logger.warning(f'CMS detection timed out for {url}')
...
def send_telegram_message(message):
	...
	# MED-11 fix: Use POST with JSON body instead of GET with message in URL
	# Prevents message leaking in proxy logs and URL length limits
...
def send_discord_message(
	...
	# CRT-05 fix: Use json instead of pickle to prevent insecure deserialization
	...
	# CRT-05 fix: Reconstruct webhook from cached JSON data instead of pickle
	cached_webhook = DISCORD_WEBHOOKS_CACHE.get(title + '_webhook') if title else None
	if cached_webhook:
		try:
			cached_webhook_data = json.loads(cached_webhook)
	...
	embed = None
	# CRT-05 fix: Reconstruct embed from cached JSON data
	cached_embed = DISCORD_WEBHOOKS_CACHE.get(title + '_embed') if title else None
	if cached_embed:
		try:
			cached_embed_data = json.loads(cached_embed)
			embed = DiscordEmbed(title=title)
			if isinstance(cached_embed_data, dict) and 'fields' in cached_embed_data:
				for field in cached_embed_data['fields']:
					embed.add_embed_field(
						name=field.get('name', ''),
						value=field.get('value', ''),
						inline=field.get('inline', False))
		except (json.JSONDecodeError, TypeError):
			embed = DiscordEmbed(title=title) if use_discord_embed else None
	...
	if embed:
		...
		# Add webhook and embed data to cache as JSON (CRT-05 fix: no pickle)
		DISCORD_WEBHOOKS_CACHE.set(title + '_webhook', json.dumps({'cached': True}))
		# Cache embed fields as JSON-serializable data
		embed_cache_data = {
			'fields': [{'name': f.get('name', ''), 'value': f.get('value', ''), 'inline': f.get('inline', False)} for f in embed.fields]
		}
		DISCORD_WEBHOOKS_CACHE.set(title + '_embed', json.dumps(embed_cache_data))
	...
	if cached_response:
		...
		if use_discord_embed and response.status_code == 200:
			# CRT-05 fix: Cache response metadata as JSON instead of pickle
			DISCORD_WEBHOOKS_CACHE.set(title, json.dumps({'status_code': response.status_code}))

	# Get status code
	if response.status_code == 429:
		# MED-27 fix: Add retry limit to prevent infinite recursion
		if _retry_count >= 3:
			logger.error('Discord rate limit: max retries (3) exceeded. Giving up.')
			return
		errors = json.loads(
...
def get_task_cache_key(func_name, *args, **kwargs):
	...
	kwargs_str = '_'.join([f'{k}={v}' for k, v in kwargs.items() if k not in PARAKANG_TASK_IGNORE_CACHE_KWARGS])
...
def get_open_ai_key():
	...
	return openai_key.key if openai_key else None


def get_netlas_key():
	netlas_key = NetlasAPIKey.objects.first()
	return netlas_key.key if netlas_key else None
...
def get_ips_from_cidr_range(target):
	MAX_CIDR_ADDRESSES = 65536  # Max /16 to prevent DoS
	try:
		network = ipaddress.IPv4Network(target, False)
		if network.num_addresses > MAX_CIDR_ADDRESSES:
			logger.error(
				f'CIDR range {target} has {network.num_addresses} addresses '
				f'(max {MAX_CIDR_ADDRESSES}). Skipping to prevent DoS.'
			)
			return []
...
def is_valid_nmap_command(cmd):
	...
	# SEC-08 fix: Added newline, carriage return, null byte, and quotes
	dangerous_chars = {';', '&', '|', '>', '<', '`', '$', '(', ')', '#', '\\', '\n', '\r', '\x00', '"', "'"}
	if any(char in cmd for char in dangerous_chars):
		return False

	# SEC-08 fix: Block -oX/-oN/-oG/-oA with path traversal
	parts = cmd.split()
	output_flags = {'-oX', '-oN', '-oG', '-oA', '-oS'}
