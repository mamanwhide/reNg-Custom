...
class CreateProjectApi(APIView):
	...
	def post(self, request):
		req = self.request
		project_name = req.data.get('name') or req.query_params.get('name')
...
class WafDetector(APIView):
	def get(self, request):
		...
		# CRT-03 fix: Sanitize URL for safe shell interpolation
...
class ToggleSubdomainImportantStatus(APIView):
	def post(self, request):
		...
		# MED-18 fix: Actually check subdomain_id before DB query
		if not subdomain_id:
			return Response({'status': False, 'message': 'No subdomain_id provided'})
...
class AddTarget(APIView):
	...
	def post(self, request):
		...
		if status:
			# Retrieve the domain to return its ID for scan initiation
			try:
	...
	def post(self, request):
		...
		for subscan_id in subscan_ids:
			try:
				...
				# HIGH-02 fix: SubScan model uses 'status' field, not 'scan_status'
...
class UninstallTool(APIView):
	...
	def get(self, request):
		...
		# HIGH-06 fix: Use safe path operations instead of shell rm -rf
		# and remove duplicate execution (was calling both run_command + apply_async)
		if 'go install' in tool.install_command:
			tool_name = tool.install_command.split('/')[-1].split('@')[0]
			bin_path = os.path.join('/go/bin', tool_name)
			if is_safe_path('/go/bin', bin_path) and os.path.isfile(bin_path):
				os.remove(bin_path)
			else:
				return Response({'status': False, 'message': 'Tool binary not found or unsafe path'})
		elif 'git clone' in tool.install_command:
			clone_path = tool.github_clone_path
			if clone_path and is_safe_path('/usr/src/github', clone_path) and os.path.isdir(clone_path):
				shutil.rmtree(clone_path, ignore_errors=True)
			else:
				return Response({'status': False, 'message': 'Clone path not found or unsafe path'})
		else:
...
class UpdateTool(APIView):
	...
	def get(self, request):
		...
		# HIGH-07 fix: Avoid shell=True and remove duplicate execution
		# (was calling both run_command + apply_async)
		if update_command.lower().strip() == 'git pull':
			tool_name = tool.install_command.rstrip('/').split('/')[-1]
			tool_dir = os.path.join('/usr/src/github', tool_name)
			if not is_safe_path('/usr/src/github', tool_dir):
				return Response({'status': False, 'message': 'Invalid tool directory path'})
			try:
...
class CMSDetector(APIView):
	def get(self, request):
		...
		try:
			...
			# MED-09 fix: Sanitize URL for safe command interpolation
...
class GetFileContents(APIView):
	def get(self, request, format=None):
		...
		if 'nuclei_config' in req.query_params:
			...
			# MED-14 fix: Use context manager for file handle
...
class SubdomainsViewSet(viewsets.ModelViewSet):
	...
	def get_queryset(self):
		...
		scan_id = req.query_params.get('scan_id')
		if scan_id:
			base_qs = Subdomain.objects.filter(scan_history__id=scan_id)
			if 'only_screenshot' in self.request.query_params:
				base_qs = base_qs.exclude(screenshot_path__isnull=True)
			# MED-03 fix: Annotate vulnerability/endpoint counts to avoid N+1 queries
			return base_qs.annotate(
				_info_count=Count('vulnerability', filter=Q(vulnerability__severity=0)),
				_low_count=Count('vulnerability', filter=Q(vulnerability__severity=1)),
				_medium_count=Count('vulnerability', filter=Q(vulnerability__severity=2)),
				_high_count=Count('vulnerability', filter=Q(vulnerability__severity=3)),
				_critical_count=Count('vulnerability', filter=Q(vulnerability__severity=4)),
				_endpoint_count=Count('endpoint', distinct=True),
			).select_related('scan_history').prefetch_related(
				'ip_addresses', 'waf', 'technologies', 'directories'
			)
