...
class DomainSerializer(serializers.ModelSerializer):
	...
	def get_organization(self, obj):
		# MED-05 fix: Single query instead of duplicate exists() + filter()
		orgs = list(Organization.objects.filter(domains__id=obj.id).values_list('name', flat=True))
...
class VisualiseEmailSerializer(serializers.ModelSerializer):
	...
	def get_description(self, email):
		if email.password:
			return email.address + " > [CREDENTIALS FOUND]"
...
class SubdomainSerializer(serializers.ModelSerializer):
	...
	# MED-03 fix: Use annotated values when available, fallback to model properties
	def get_endpoint_count(self, subdomain):
		return getattr(subdomain, '_endpoint_count', subdomain.get_endpoint_count)

	def get_info_count(self, subdomain):
		return getattr(subdomain, '_info_count', subdomain.get_info_count)

	def get_low_count(self, subdomain):
		return getattr(subdomain, '_low_count', subdomain.get_low_count)

	def get_medium_count(self, subdomain):
		return getattr(subdomain, '_medium_count', subdomain.get_medium_count)

	def get_high_count(self, subdomain):
		return getattr(subdomain, '_high_count', subdomain.get_high_count)

	def get_critical_count(self, subdomain):
		return getattr(subdomain, '_critical_count', subdomain.get_critical_count)
	...
	# LOW-08 fix: Use .count() instead of len() for QuerySet counting
