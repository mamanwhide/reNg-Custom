...
/**
 * CRT-08/09/10 fix: HTML entity escaping to prevent DOM XSS.
 * All user-controlled or API-returned data MUST be passed through
 * this function before being inserted into HTML via template literals.
 */
...
/**
 * Escape a string for safe use inside an HTML attribute (e.g., onclick handlers).
 * Prevents attribute breakout via quotes.
 */
...
function parse_technology(data, color, scan_id = null, domain_id=null) {
	...
	var data_with_span = "";
	for (var key in data) {
		var safeName = escapeAttr(data[key]['name']);
		var displayName = escapeHtml(data[key]['name']);
		if (scan_id) {
			data_with_span += badge + ` onclick="get_tech_details('${safeName}', ${scan_id}, domain_id=null)">` + displayName + "</span>";
		} else if (domain_id) {
			data_with_span += badge + ` onclick="get_tech_details('${safeName}', scan_id=null, domain_id=domain_id)">` + displayName + "</span>";
		}
	}
	return data_with_span;
}
	...
	var interesting_subdomain_table = $('#interesting_subdomains').DataTable({
		"drawCallback": function(settings, start, end, max, total, pre) {
			...
			if (this.fnSettings().fnRecordsTotal() == 0) {
				...
				// <div class="card-body">
				// paraKang could not identify any interesting subdomains. You can customize interesting subdomain keywords <a href="/scanEngine/interesting/lookup/">from here</a> and this section would be automatically updated.
				// </div>
				// </div>`);
			} else {
				// show nav bar
			...
			if (response['subscan']['task'] == 'port_scan') {
				...
				for (var ip in response['result']) {
					...
					for (var port_obj in response['result'][ip]['ports']) {
						...
						$(`#${id_name}`).append(`<li><span class="ms-1 mt-1 me-1 badge badge-soft-${port_color}">${escapeHtml(String(port['number']))}</span>/<span class="ms-1 mt-1 me-1 badge badge-soft-${port_color}">${escapeHtml(port['service_name'])}</span>/<span class="ms-1 mt-1 me-1 badge badge-soft-${port_color}">${escapeHtml(port['description'])}</span></li>`);
					}
				}
...
function render_directories_in_xl_modal(directory_count, subdomain_name, result) {
	...
	for (var dir_obj in result) {
		...
		$('#directory_tbody').append(`
			<tr>
			<td><a href="${escapeAttr(dir.url)}" target="_blank">${escapeHtml(atob(dir.name))}</a></td>
			<td class="text-center">${get_http_status_badge(dir.http_status)}</td>
			<td>${escapeHtml(String(dir.length))}</td>
			<td>${escapeHtml(String(dir.lines))}</td>
			<td>${escapeHtml(String(dir.words))}</td>
			</tr>
		`);
	}
		...
		if (data['status']) {
			...
			for (var result in data['results']) {
				...
				if (result_obj.status == 0) {
					...
					error_message = `</br><span class="text-danger">Error: ${escapeHtml(result_obj.error_message)}`;
...
function display_whois_on_modal(response, show_add_target_btn=false) {
			...
			<div class="tab-pane fade active show" id="v-pills-domain" role="tabpanel" aria-labelledby="v-pills-domain-tab" data-simplebar style="min-height: 300px;">
				<div class="row">
					<div class="col-4">
						...
						<h5>${escapeHtml(response.target)}</h5>
					</div>
					<div class="col-4">
						<small class="sub-header">Dnssec</small>
						<h5>${escapeHtml(response.dnssec)}</h5>
					</div>
					<div class="col-4">
						<small class="sub-header">Geolocation</small>
						<h5>${escapeHtml(response.geolocation_iso)}
						<span class="ms-2 fi fi-${escapeAttr(response.geolocation_iso)}"></span></h5>
					</div>
				</div>
				<div class="row">
					<div class="col-4">
						<small class="sub-header">Created</small>
						<h5>${escapeHtml(response.created)}</h5>
					</div>
					<div class="col-4">
						<small class="sub-header">Updated</small>
						<h5>${escapeHtml(response.updated)}</h5>
					</div>
					<div class="col-4">
						<small class="sub-header">Expires</small>
						<h5>${escapeHtml(response.expires)}</h5>
					</div>
				</div>
				<div class="row">
					<div class="col-6">
						<small class="sub-header">Whois Server</small>
						<h5>${escapeHtml(response.whois_server)}</h5>
					</div>
				</div>
				<div class="row">
					<div class="col-4">
						<small class="sub-header">Registrar Name</small>
						<h5>${escapeHtml(response.registrar.name)}</h5>
					</div>
					<div class="col-4">
						<smal class="sub-header"l>Registrar Phone</small>
						<h5>${escapeHtml(response.registrar.phone)}</h5>
					</div>
					<div class="col-4">
						<small class="sub-header">Registrar Email</small>
						<h5>${escapeHtml(response.registrar.email)}</h5>
					</div>
				</div>`;
				...
				for (var status in response.status) {
					...
					if (status_object.includes('prohibited')) {
						content += `<span class="badge badge-soft-danger me-1 mt-1">${escapeHtml(status_object)}</span>`;
					}
					else {
						content += `<span class="badge badge-soft-info mt-1 me-1">${escapeHtml(status_object)}</span>`;
					}
				}
				content += `
				</div>
							...
							<div class="table-responsive">
								<table class="table mb-0">
									<tbody>
										<tr class="table-info">
											<td><b>ID</b></td>
											<td><span class="fe-user"></span>&nbsp;${escapeHtml(response.registrant.id)}</td>
										</tr>
										<tr class="">
											<td><b>Name</b></td>
											<td><span class="fe-user"></span>&nbsp;${escapeHtml(response.registrant.name)}</td>
										</tr>
										<tr class="table-primary">
											<td><b>Organization</b></td>
											<td><span class="fe-briefcase"></span>&nbsp;${escapeHtml(response.registrant.organization)}</td>
										</tr>
										<tr class="">
											<td><b>Email</b></td>
											<td><span class="fe-mail"></span>&nbsp;${escapeHtml(response.registrant.email)}</td>
										</tr>
										<tr class="table-info">
											<td><b>Phone/Fax</b></td>
											<td>
												<span class="fe-phone"></span>&nbsp;${escapeHtml(response.registrant.phone)}
												<span class="fe-printer"></span>&nbsp;${escapeHtml(response.registrant.fax)}
											</td>
										</tr>
										<tr class="">
											<td><b>Address</b></td>
											<td><span class="fe-home"></span>&nbsp;${escapeHtml(response.registrant.address)}</td>
										</tr>
										<tr class="table-danger">
											<td><b>Address</b></td>
											<td><b>City: </b>${escapeHtml(response.registrant.city)} <b>State: </b>${escapeHtml(response.registrant.state)} <b>Zip Code: </b>${escapeHtml(response.registrant.zipcode)} <b>Country:
											</b>${escapeHtml(response.registrant.country)} </td>
										</tr>
									</tbody>
								</table>
							</div>
							...
							<div class="table-responsive">
								<table class="table mb-0">
									<tbody>
										<tr class="table-info">
											<td><b>ID</b></td>
											<td><span class="fe-user"></span>&nbsp;${escapeHtml(response.admin.id)}</td>
										</tr>
										<tr class="">
											<td><b>Name</b></td>
											<td><span class="fe-user"></span>&nbsp;${escapeHtml(response.admin.name)}</td>
										</tr>
										<tr class="table-primary">
											<td><b>Organization</b></td>
											<td><span class="fe-briefcase"></span>&nbsp;${escapeHtml(response.admin.organization)}</td>
										</tr>
										<tr class="">
											<td><b>Email</b></td>
											<td><span class="fe-mail"></span>&nbsp;${escapeHtml(response.admin.email)}</td>
										</tr>
										<tr class="table-info">
											<td><b>Phone/Fax</b></td>
											<td>
												<span class="fe-phone"></span>&nbsp;${escapeHtml(response.admin.phone)}
												<span class="fe-printer"></span>&nbsp;${escapeHtml(response.admin.fax)}
											</td>
										</tr>
										<tr class="">
											<td><b>Address</b></td>
											<td><span class="fe-home"></span>&nbsp;${escapeHtml(response.admin.address)}</td>
										</tr>
										<tr class="table-danger">
											<td><b>Address</b></td>
											<td><b>City: </b>${escapeHtml(response.admin.city)} <b>State: </b>${escapeHtml(response.admin.state)} <b>Zip Code: </b>${escapeHtml(response.admin.zipcode)} <b>Country:
											</b>${escapeHtml(response.admin.country)} </td>
										</tr>
									</tbody>
								</table>
							</div>
							...
							<div class="table-responsive">
								<table class="table mb-0">
									<tbody>
										<tr class="table-info">
											<td><b>ID</b></td>
											<td><span class="fe-user"></span>&nbsp;${escapeHtml(response.technical_contact.id)}</td>
										</tr>
										<tr class="">
											<td><b>Name</b></td>
											<td><span class="fe-user"></span>&nbsp;${escapeHtml(response.technical_contact.name)}</td>
										</tr>
										<tr class="table-primary">
											<td><b>Organization</b></td>
											<td><span class="fe-briefcase"></span>&nbsp;${escapeHtml(response.technical_contact.organization)}</td>
										</tr>
										<tr class="">
											<td><b>Email</b></td>
											<td><span class="fe-mail"></span>&nbsp;${escapeHtml(response.technical_contact.email)}</td>
										</tr>
										<tr class="table-info">
											<td><b>Phone/Fax</b></td>
											<td>
												<span class="fe-phone"></span>&nbsp;${escapeHtml(response.technical_contact.phone)}
												<span class="fe-printer"></span>&nbsp;${escapeHtml(response.technical_contact.fax)}
											</td>
										</tr>
										<tr class="">
											<td><b>Address</b></td>
											<td><span class="fe-home"></span>&nbsp;${escapeHtml(response.technical_contact.address)}</td>
										</tr>
										<tr class="table-danger">
											<td><b>Address</b></td>
											<td><b>City: </b>${escapeHtml(response.technical_contact.city)} <b>State: </b>${escapeHtml(response.technical_contact.state)} <b>Zip Code: </b>${escapeHtml(response.technical_contact.zipcode)} <b>Country:
											</b>${escapeHtml(response.technical_contact.country)} </td>
										</tr>
									</tbody>
								</table>
							</div>
				...
				<div class="tab-pane fade" id="v-pills-dns" role="tabpanel" aria-labelledby="v-pills-dns-tab" data-simplebar style="min-height: 300px;">
					...
					for (var a in response.dns.a) {
						...
						content += `<span class="badge badge-soft-primary me-1 mt-1">${escapeHtml(a_object)}</span>`;
					}
					content += `<h4>MX Records</h4>`;

					for (var mx in response.dns.mx) {
						var mx_object = response.dns.mx[mx];
						content += `<span class="badge badge-soft-primary me-1 mt-1">${escapeHtml(mx_object)}</span>`;
					}
					content += `<h4>TXT Records</h4>`;
					for (var txt in response.dns.txt) {
						var txt_object = response.dns.txt[txt];
						content += `<span class="badge badge-soft-secondary me-1 mt-1">${escapeHtml(txt_object)}</span>`;
					}
					content += `</div>`;
					...
					content += `<div class="tab-pane fade" id="v-pills-history" role="tabpanel" aria-labelledby="v-pills-history-tab" data-simplebar style="max-height: 300px; min-height: 300px;">
						<div class="alert alert-success">${escapeHtml(response.historical_ips.length)} Historical Ips</div>
						<table id="basic-datatable" class="table dt-responsive w-100">
							...
							for (var ip in response.historical_ips) {
								...
								content += `<tr>
									<td><b>${escapeHtml(ip_object.ip)}</b></td>
									<td>${escapeHtml(ip_object.location)}</td>
									<td>${escapeHtml(ip_object.owner)}</td>
									<td>${escapeHtml(ip_object.last_seen)}</td>
								</tr>`;
							}
					...
					content += `<div class="alert alert-success">${escapeHtml(response.nameservers.length)} NameServers identified</div>`;

					for (var ns in response.nameservers) {
						var ns_object = response.nameservers[ns];
						content += `<span class="badge badge-soft-primary me-1 mt-1">${escapeHtml(ns_object)}</span>`;
					}
					...
					if (response.related_tlds.length > 0) {
						for (var domain in response.related_tlds) {
							...
							content += `<span class="badge badge-soft-primary badge-link waves-effect waves-light me-1" data-toggle="tooltip" title="Add ${escapeAttr(dom_object)} as target." onclick="add_target('${escapeAttr(dom_object)}')">${escapeHtml(dom_object)}</span>`;
						}
					}
					else{
	...
	if (show_add_target_btn) {
		content += `<div class="text-center">
			<button class="btn btn-primary float-end mt-4" type="submit" id="search_whois_toolbox_btn" onclick="add_target('${escapeAttr(response['target'])}')">Add ${escapeHtml(response['target'])} as target</button>
		</div>`
	}
...
function get_ips(scan_id=null, domain_id=null){
	...
	$.getJSON(url, function(data) {
		...
		for (var val in data['ips']){
			...
			if (scan_id) {
				$("#ip-address").append(`<span class='badge badge-soft-${badge_color}  m-1 badge-link' data-toggle="tooltip" title="${escapeAttr(ip['ports'].length + ' Ports Open.')}" onclick="get_ip_details('${escapeAttr(ip['address'])}', scan_id=${scan_id}, domain_id=null)">${escapeHtml(ip['address'])}</span>`);
			}
			else if (domain_id) {
				$("#ip-address").append(`<span class='badge badge-soft-${badge_color}  m-1 badge-link' data-toggle="tooltip" title="${escapeAttr(ip['ports'].length + ' Ports Open.')}" onclick="get_ip_details('${escapeAttr(ip['address'])}', scan_id=null, domain_id=${domain_id})">${escapeHtml(ip['address'])}</span>`);
			}
...
function get_technologies(scan_id=null, domain_id=null){
	...
	$.getJSON(url, function(data) {
		...
		for (var val in data['technologies']){
			...
			if (scan_id) {
				$("#technologies").append(`<span class='badge badge-soft-primary  m-1 badge-link' data-toggle="tooltip" title="${escapeAttr(tech['count'] + ' Subdomains use this technology.')}" onclick="get_tech_details('${escapeAttr(tech['name'])}', scan_id=${scan_id}, domain_id=null)">${escapeHtml(tech['name'])}</span>`);
			}
			else if (domain_id) {
				$("#technologies").append(`<span class='badge badge-soft-primary  m-1 badge-link' data-toggle="tooltip" title="${escapeAttr(tech['count'] + ' Subdomains use this technology.')}" onclick="get_tech_details('${escapeAttr(tech['name'])}', scan_id=null, domain_id=${domain_id})">${escapeHtml(tech['name'])}</span>`);
			}
		}
...
function get_ports(scan_id=null, domain_id=null){
	...
	$.getJSON(url, function(data) {
		...
		for (var val in data['ports']){
			...
			if (scan_id) {
				$("#ports").append(`<span class='badge badge-soft-${badge_color}  m-1 badge-link' data-toggle="tooltip" title="${escapeAttr(port['description'])}" onclick="get_port_details('${escapeAttr(String(port['number']))}', scan_id=${scan_id}, domain_id=null)">${escapeHtml(String(port['number']))}/${escapeHtml(port['service_name'])}</span>`);
			}
			else if (domain_id){
				$("#ports").append(`<span class='badge badge-soft-${badge_color}  m-1 badge-link' data-toggle="tooltip" title="${escapeAttr(port['description'])}" onclick="get_port_details('${escapeAttr(String(port['number']))}', scan_id=null, domain_id=${domain_id})">${escapeHtml(String(port['number']))}/${escapeHtml(port['service_name'])}</span>`);
			}
		}
	...
	fetch('/api/fetch/most_vulnerable/?format=json', {
		...
		if (response.status) {
			...
			$('#most_vulnerable_target_div').append(`
				...
				Once the vulnerability scan is performed, paraKang will identify the most vulnerable targets.</div>
			`);
		}
	});
}
	...
	fetch('/api/fetch/most_common_vulnerability/?format=json', {
		...
		if (response.status) {
			...
			$('#most_common_vuln_div').append(`
				...
				Once the vulnerability scan is performed, paraKang will identify the Most Common Vulnerabilities.</div>
			`);
		}
	});
}
...
function add_project_modal(){
	Swal.fire({
		...
		showLoaderOnConfirm: true,
		preConfirm: (name) => {
			return fetch(`/api/action/create/project`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					'X-CSRFToken': getCookie('csrftoken')
...
function render_gpt_vuln_modal(data, title){
	...
	$('#modal-content').empty();
	$('#modal-footer').empty();
	$('#modal_title').html(`Vulnerability detail for ${escapeHtml(title)}`);
...
async function show_attack_surface_modal(id){
	...
	try {
		...
		Swal.close();
		if (data.status) {
			$('#modal_title').html(`Attack Surface Suggestion for ${escapeHtml(data.subdomain_name)} (BETA)`);
			$('#modal-content').empty();
			// CRT-10 fix: Escape LLM response before rendering
			$('#modal-content').append(escapeHtml(data.description).replace(new RegExp('\r?\n','g'), '<br />'));
			$('#modal_dialog').modal('show');
		}
		else{
			Swal.close();
