    ...
    if request.method == "POST":
        if form.is_valid() and 'upload_file' in request.FILES:
            ...
            if txt_file.content_type == 'text/plain':
                ...
                # HIGH-BUG-08 fix: Validate short_name to prevent path traversal
                short_name = form.cleaned_data['short_name']
                short_name = re.sub(r'[^a-zA-Z0-9_\-.]', '', os.path.basename(short_name))
                if not short_name or '..' in short_name:
                    messages.add_message(request, messages.ERROR, 'Invalid wordlist name')
                    return http.HttpResponseRedirect(reverse('wordlist_list', kwargs={'slug': slug}))
                wordlist_path = os.path.join('/usr/src/wordlist/', short_name + '.txt')
                if not is_safe_path('/usr/src/wordlist', wordlist_path):
                    messages.add_message(request, messages.ERROR, 'Invalid wordlist path')
                    return http.HttpResponseRedirect(reverse('wordlist_list', kwargs={'slug': slug}))
...
def tool_specific_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if 'gfFileUpload[]' in request.FILES:
            ...
            # CRT-04 fix: Validate YAML before writing to config file
            config_content = request.POST.get('nuclei_config_text_area')
            is_valid, error = validate_yaml_config(config_content)
            if not is_valid:
                messages.add_message(request, messages.ERROR, f'Invalid nuclei config: {error}')
                return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
            ...
            # CRT-04 fix: Validate YAML before writing to config file
            config_content = request.POST.get('subfinder_config_text_area')
            is_valid, error = validate_yaml_config(config_content)
            if not is_valid:
                messages.add_message(request, messages.ERROR, f'Invalid subfinder config: {error}')
                return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
            ...
            # CRT-04 fix: Validate YAML before writing to config file
            config_content = request.POST.get('naabu_config_text_area')
            is_valid, error = validate_yaml_config(config_content)
            if not is_valid:
                messages.add_message(request, messages.ERROR, f'Invalid naabu config: {error}')
                return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
            ...
            # MED-05 fix: Validate INI config before writing
            config_content = request.POST.get('amass_config_text_area')
            is_valid, error = validate_ini_config(config_content)
            if not is_valid:
            ...
            # CRT-04 fix: Validate YAML before writing to config file
            config_content = request.POST.get('theharvester_config_text_area')
            is_valid, error = validate_yaml_config(config_content)
            if not is_valid:
                messages.add_message(request, messages.ERROR, f'Invalid theHarvester config: {error}')
                return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
...
def add_tool(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # CRT-03 fix: Validate install command against whitelist
            is_valid, error_msg = validate_install_command(install_command)
            if not is_valid:
                messages.add_message(
                    request,
                    messages.ERROR,
                    f'Install command rejected for security: {error_msg}')
            ...
            if 'git clone' in install_command:
                ...
                if not project_name:
                    messages.add_message(request, messages.ERROR, 'Invalid project name')
                    return http.HttpResponseRedirect(reverse('tool_arsenal', kwargs={'slug': slug}))
                # HIGH-03 fix: Chain git clone and pip install properly
                # First clone the repo, then install requirements if they exist
                clone_dir = '/usr/src/github/' + project_name
                install_command = install_command + ' ' + clone_dir + ' && pip3 install -r ' + clone_dir + '/requirements.txt'
                github_clone_path = clone_dir

            # CRT-03 fix: Only execute async (removed duplicate sync execution)
