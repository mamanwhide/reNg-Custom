        ...
        try:
                        ...
                        if not Domain.objects.filter(name=domain_name).exists():
                            ...
                            if organization_name:
                                ...
                                if Organization.objects.filter(name=organization_name).exists():
                                    # CRT-15 fix: Use proper query instead of undefined organization_query
            ...
            # Import from txt / csv
                ...
                if txt_file:
                    ...
                    # CRT-14 fix: Use == (comparison) instead of = (assignment)
...
def delete_target(request, id):
    ...
    if request.method == "POST":
        # CRT-02 fix: Use safe file deletion instead of shell rm -rf
        safe_delete_scan_results(obj.name)
        obj.delete()
    ...
    if request.method == "POST":
        ...
        for key, value in request.POST.items():
            if key != "list_target_table_length" and key != "csrfmiddlewaretoken":
                # MED-07 fix: Clean up scan results before deleting domain
...
def update_organization(request, slug, id):
    ...
    form = UpdateOrganizationForm(project=slug)
    if request.method == "POST":
        print(request.POST.getlist("domains"))
        form = UpdateOrganizationForm(request.POST, instance=organization, project=slug)
        if form.is_valid():
            data = form.cleaned_data
    ...
    context = {
        ...
        # LOW-05 fix: Use json.dumps instead of mark_safe to prevent XSS
