...
class AddOrganizationForm(forms.Form):
    ...
    def clean_name(self):
        ...
        # MED-23 fix: Scope uniqueness check to current project
        if Organization.objects.filter(
            name=data,
            project__slug=self._project_slug
        ).exists():
            raise forms.ValidationError(f"{data} Organization already exists in this project")
        return data
...
class UpdateOrganizationForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if project:
            ...
            self.fields['domains'].choices = [(domain.id, domain.name) for domain in Domain.objects.none()]
