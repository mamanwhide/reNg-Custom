...
def index(request, slug):
    ...
    # Update cached context with non-cacheable data
    context.update({
...
def admin_interface_update(request, slug):
    mode = request.GET.get('mode') if request.method == 'GET' else request.POST.get('mode', request.GET.get('mode'))
    user_id = request.GET.get('user') if request.method == 'GET' else request.POST.get('user', request.GET.get('user'))
    if user_id:
        UserModel = get_user_model()
        user = UserModel.objects.get(id=user_id)
    if request.method == 'GET':
        pass  # GET requests should not perform state changes
...
def onboarding(request):
    ...
    if request.method == "POST":
        ...
        # LOW-04/MED-10 fix: Redirect after POST to prevent API key leakage
        # and avoid double POST on refresh
        if not error:
            project_obj = Project.objects.filter(slug=slug).first()
            if project_obj:
                return HttpResponseRedirect(reverse('dashboardIndex', kwargs={'slug': slug}))
    ...
    # LOW-04 fix: Cache HackerOneAPIKey query to avoid duplicate DB calls
    hackerone_obj = HackerOneAPIKey.objects.first()
    context['hackerone_key'] = hackerone_obj.key if hackerone_obj else ''
    context['hackerone_username'] = hackerone_obj.username if hackerone_obj else ''
